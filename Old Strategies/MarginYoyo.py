    ...
    for line in response.iter_lines():
        if line:
            ...
            try:
                ...
                print(e)
                continue
            if (prev_close < cur_close) and (cur_close <= (float(loaded_line['High'])) - .01):
                print('prev_close: ', prev_close, '\n', 'cur_close: ', cur_close)
                print('Buying Condition Triggered')
                return [prev_close,cur_close]
            print('prev_close: ', prev_close, '\n', 'cur_close: ', cur_close)
...
def market_status():
    ...
    if schedule.empty:
        ...
        if (schedule['market_open'][0] + timedelta(minutes=60)) > datetime.now(pytz.utc):
            return False
        elif (schedule['market_close'][0] - timedelta(minutes=60)) < datetime.now(pytz.utc):
            return False
        else:
            return True
